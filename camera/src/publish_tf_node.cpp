...
void PublisherSubscriber<tf2_msgs::TFMessage, tf2_msgs::TFMessage>::subscriberCallback(const tf2_msgs::TFMessage::ConstPtr& receivedMsg)
{
  ros::Rate r(10); // 10 hz
  while (ros::ok())
  {
    ROS_INFO("Sending the received message on 'transform' topic");
    publisherObject.publish(receivedMsg->transforms);
...
int main(int argc, char **argv)
{
  ...
  PublisherSubscriber<tf2_msgs::TFMessage, tf2_msgs::TFMessage> parrot("tf", "tf_static", 1);
  ros::spin();
}
